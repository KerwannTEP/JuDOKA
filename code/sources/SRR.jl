"""
    DRR_JJnnp(n,np,a,j,Table_dKdJ_arg=Table_dKdJ,tabSMARes_arg=tabSMARes)
...
- `tabSMARes_arg=tabSMARes`: Static container for the resonant lines, contains the edge of the domain [amin,amax] of the resonant line.
"""
function DRR_JJnnp(n::Int64,np::Int64,a::Float64,j::Float64,Table_K_arg=Table_K,tabSMARes_arg=tabSMARes)
    #####
    ...
    apmin1, apmax1 = tabSMARes_arg[1], tabSMARes_arg[2] # Boundary of the SMA-region where the resonant line lies
    dalphap1 = log(apmax1/apmin1)/(nbResPoints) # Step distance in log-space
    
    if (ResPoints == 2) # If second resonance line
        apmin2, apmax2 = tabSMARes_arg[3], tabSMARes_arg[4] # Boundary of the SMA-region where the resonant line lies
    ...
    for alphap1 = range(log(apmin1),length=nbResPoints,log(apmax1)-dalphap1) # Uniform logarithmic spacing of the SMA !! ATTENTION, we do not go through the last element, as we will sum in the middle of the interval
        ...
        res += dalphap1*ap*FtotBath(ap,jp)*AnnpSQ(n,np,a,j,ap,jp,Table_K_arg)/(abs(dnupdj(ap,jp))) # Local contribution !! ATTENTION, not to forget the ap
    end
    if (ResPoints == 2) # If two resonance lines
        for alphap2 = range(log(apmin2),length=nbResPoints,log(apmax2)-dalphap2) # Uniform logarithmic spacing of the SMA !! ATTENTION, we do not go through the last element, as we will sum in the middle of the interval
            ...
            jp = getjRes(np,ap,omega) # Finding the location of the resonant jp
            res += dalphap2*ap*FtotBath(ap,jp)*AnnpSQ(n,np,a,j,ap,jp,Table_K_arg)/(abs(dnupdj(ap,jp))) # Local contribution !! ATTENTION, not to forget the ap
        end
    end
...
"""
    DRR_JJ(a,j,Table_dKdJ_arg=Table_dKdJ,tabSMARes_arg=tabSMARes)
...
"""
function DRR_JJ(a::Float64,j::Float64,Table_K_arg=Table_K,tabSMARes_arg=tabSMARes)
    res = 0.0 # Initialisation of the result
    #####
    for iPair=1:nbResPairs # Loop over the resonant pairs
        ...
        res += DRR_JJnnp(n,np,a,j,Table_K_arg,tabSMARes_arg) # Adding the contribution from the resonance pair (n,np)
    end
    #####
...
"""
function DRR_jj(a::Float64,j::Float64,Table_K_arg=Table_K,tabSMARes_arg=tabSMARes)
    return DRR_JJ(a,j,Table_K_arg,tabSMARes_arg)/((Jc(a))^(2)) # Output
end
